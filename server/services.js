// Update de profile photo with the fb photo.
...
// update: now we create one collection for user to store the rates
Accounts.onCreateUser(function(options, user) {
  if(options.profile) {
    options.profile.picture = getFbPicture(user.services.facebook.accessToken);
    user.profile = options.profile; // We still want the default 'profile' behavior.
  }
  Rates.insert({'userId': user._id, 'rated': [], 'likes': [], 'list_rated': []});
  return user;
});
